...
bee 엔진의 **매니페스트 파생 차트**. 규칙 1("파생은 차트가 한다")의 거처 — 모듈의 논리 선언
(`module.yaml` + `values-<env>`)을 받아 k8s 매니페스트로 렌더한다. **특정 플랫폼·제품·org 지식 0**
(bitcert·attest 등 결합 없음 — 순수 제네릭). G57 에서 입주자(core-infra)에서 추출돼 자기 집을 가졌다.

## 위치 (G5 정정 — 레포셋 3→4, G57)

엔진 레포셋: `bee-cli`(오케스트레이터) · **`bee-chart`(이 레포 — 파생 차트)** ⊥ 인스턴스: `<platform>-core-infra`
(platform.yaml·argocd·substrate) · `<platform>-snapshot`(렌더 SoT) · 모듈 N. 한 입주자 레포 안에 있던
차트가 엔진 릴리스를 인질로 잡던 구조를 해소 — 차트는 자기 버전·릴리스 주기를 가진다.
...
## 코덱 (provider-rendering, G40·G44)

`_routing.tpl`(routing: kong↔nginx) · `_db.tpl`(db: postgres↔mysql) — capability→provider→템플릿 dispatch.
seam 인터페이스 = 동결 어휘(`spec.routing`·`spec.db`). 새 provider 분기는 *엔진 기여*(additive, 모든
플랫폼 공유) — per-platform fork 아님.

## 스키마 (G9)

`values.schema.json` = values 계약. 검증 = helm + CI lint(`conftest`), **CLI 검증 안 함**(규칙 2).
